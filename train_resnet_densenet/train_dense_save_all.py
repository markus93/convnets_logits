...
PATH_L = path.join("..", "..", "logits_epoch_dense")
...
def train(seed = 13, num_classes = 10):
    ...
    checkpointer = ModelCheckpoint(path.join(PATH_W, 'weights_densenet40_c10_seed13_epoch{epoch:03d}.hdf5'), verbose=1, save_best_only=False, period=1)
    callbacks = [LearningRateScheduler(scheduler), checkpointer]
...
def gen_logits(seed = 13, num_classes = 10, epochs = 300):

    for i in range(0, epochs):

        print("Epoch:", i)    

        weights_file = path.join(PATH_W, 'weights_densenet40_c%i_seed%i_epoch%03i.hdf5' % (num_classes, seed, i+1))  # TODO check whether weight file exists   
        file_path = path.join(PATH_L, "logits_densenet40_c%i_seed%i_epoch%03i" % (num_classes, seed, i+1))
